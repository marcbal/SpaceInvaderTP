	...
	public void performMove(long delta, EntitiesManager entMan) {
		...
		for(Entity entity : entMan.getEntitiesList()) {
	    	if (entity instanceof EntityEnnemy){
	    		...
	    		if(upDown == 'd' && (entity.getPosition().y > Game.gameInstance.getWindowHeight() - 10 - entity.getBoundingBox().height))
	    			upDown = 'u';
		...
		if (countOfDeath>10) {
			...
			for(Entity entity : entMan.getEntitiesList()) {
				if(entity instanceof EntityEnnemy) {
					entity.setSpeed(entity.getSpeed().add(new Vector2d(20, 20)));
				}
