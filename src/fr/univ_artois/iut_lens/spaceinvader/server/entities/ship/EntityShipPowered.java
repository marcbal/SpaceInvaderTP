...
public class EntityShipPowered extends EntityShip {
	...
	public void shoot(long currentTime) {
		...
		entitiesManager.add(new EntityShotFromAllyBasic(new Vector2d(position.x+getBoundingBox().width/2-10, position.y), new Vector2d(-100, -1000), entitiesManager));
		entitiesManager.add(new EntityShotFromAllyBasic(new Vector2d(position.x+getBoundingBox().width/2, position.y), new Vector2d(100, -1000), entitiesManager));
